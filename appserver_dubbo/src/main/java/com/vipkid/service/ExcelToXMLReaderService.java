...
@Service
public class ExcelToXMLReaderService {
	...
	public Response doInitForCourse() throws JDOMException, IOException {
		readCourse();
		return  new Response(HttpStatus.OK.value());
	}
	
	public Response doInitForSlide() throws JDOMException, IOException {
		String path = this.getClass().getResource("/data/slides.xml").getPath();
		readSlide(path);
		return  new Response(HttpStatus.OK.value());
	}
	
	public Response doInitForSlideForTest() throws JDOMException, IOException {
		String path =  this.getClass().getResource("/data/slidesForTest.xml").getPath();
		readSlide(path);
		return new Response(HttpStatus.OK.value());
	}
	
	@SuppressWarnings("unchecked")
	public void readSlide(String path) throws JDOMException, IOException {
		SAXBuilder saxBuilder = new SAXBuilder(); 
		File xmlFile = new File(path);
		try{
			Document document = (Document) saxBuilder.build(xmlFile);
            Element workbookElement = document.getRootElement();
            Namespace workbookNamespace = workbookElement.getNamespace();
            
                        
            Element worksheetElement = workbookElement.getChild("Worksheet", workbookNamespace); 
            ...
            for(int i = 1; i < rowElements.size(); i++){ // 从第二行开始
            	...
            	Namespace rawNamespace = rowElement.getNamespace();
            	List<Element> cellElements = rowElement.getChildren("Cell", rawNamespace);
            	
                // 如果没有数据了就跳出
                Element findCell = cellElements.get(0);
                Namespace findCellNamespace = findCell.getNamespace();
                Element findDataElement = findCell.getChild("Data", findCellNamespace);
                ...
                for(int j = cellElements.size() - 1; j >= 0; j--){
                    ...
                    System.out.println(stringContent + " " + "count:" + j + " " + "row:" + " " + i );
                    ...
                    case 1:
                    	slide = new Slide();
                    	if(stringContent.equals("N/A")) {
                    		slide.setTips("");
                    	}else {
                    		slide.setTips(stringContent);
                    	}                    	
                    	break;
                    // slide page
                    case 2:
                    	slide.setPage(Integer.parseInt(stringContent));
                    	break;
                    // lesson SerialNumber
                    case 3:
                    	Slide findSlide = slideRepository.findByLessonSerialNumberAndSlidePage(stringContent, slide.getPage());
                    	if(findSlide == null) {
                    		findPPT = pptRepository.findByLessonSerialNumberAndType(stringContent, Resource.Type.PPT);
                    		if(findPPT != null){
                    			slide.setPPT(findPPT);
                    			slideRepository.create(slide);
                    		}else {
//                    			 throw new IllegalArgumentException("ppt data error");
                    		}
                    	}else {
                    		findSlide.setTips(slide.getTips());
                    		slideRepository.update(findSlide);
                    	}
	...
	@SuppressWarnings("unchecked")
	public void readCourse() throws JDOMException, IOException {
		SAXBuilder saxBuilder = new SAXBuilder();
        URL fileURL = ExcelToXMLReaderService.class.getResource("/data/courses.xml");
		File xmlFile = new File(fileURL.getFile());
		try{
			Document document = (Document)saxBuilder.build(xmlFile);
