...
public class ImageProcessAPI {
	...
	public static boolean shrink(final String url, final String height, final String width, final String type){
	    ...
	    httpClient.close();
	    	...
	    	logger.error("exception when processing image: {}", e.getMessage());
			throw new InternalServerErrorServiceException("exception when processing image: {}", e.getMessage());
		} finally{
			try {
				httpClient.close();
			} catch (IOException e) {
